...
src34=repo/'.printcheck-alpha34'
src35=repo/'.printcheck-alpha35'
if src35.exists():
    shutil.rmtree(src35)
shutil.copytree(src34,src35)

parts=sorted((pb/'alpha35_patch').glob('part*.b64'))
if not parts:
    raise RuntimeError('alpha35 patch chunks missing')
enc=b''.join(p.read_bytes().replace(b'\n',b'').replace(b'\r',b'') for p in parts)
patch=gzip.decompress(base64.b64decode(enc,validate=True))
if hashlib.sha256(patch).hexdigest()!='c2595cdefe114eb580f33ebf546e8f59deff7e88b26f1e0bbb4d89f046949652':
    raise RuntimeError('alpha35 patch sha mismatch')

pp=repo/'.alpha35.patch'
pp.write_bytes(patch)
try:
    subprocess.run(['patch','-p1','--batch','--forward','-i',str(pp)],cwd=src35,check=True)
finally:
...
mf=(src35/'app/src/main/AndroidManifest.xml').read_text()
m=(src35/'app/src/main/java/ru/printcheck/android/MainActivity.java').read_text()
q=(src35/'app/src/main/java/ru/printcheck/android/QuickCheckActivity.java').read_text()
qe=(src35/'app/src/main/java/ru/printcheck/android/QuickCheckEngine.java').read_text()
pr=(src35/'app/src/main/java/ru/printcheck/android/PdfReportWriter.java').read_text()
pc=(src35/'app/src/main/java/ru/printcheck/android/ProductColorLogic.java').read_text()
ppol=(src35/'app/src/main/java/ru/printcheck/android/ProductColorPolicy.java').read_text()
...
if bad:
    raise RuntimeError('alpha35 invariant failure: '+', '.join(bad))

signing=src35/'signing/printcheck-alpha-test.p12'
...
if not signing.is_file():
    if os.environ.get('PRINTCHECK_SKIP_SIGNING_CHECK')!='1':
        raise RuntimeError('alpha35 signing key missing')
elif hashlib.sha256(signing.read_bytes()).hexdigest()!=expected:
    raise RuntimeError('alpha35 signing identity changed')
